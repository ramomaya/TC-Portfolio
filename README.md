# Portfolio

## 🌱 REKO – Paikallisen ruoan verkkosovellus

REKO on verkkosovellus, joka yhdistää paikalliset ruoantuottajat ja kuluttajat yhteen selkeään palveluun. Sovelluksen avulla kuluttajat voivat selata lähiruokatuotteita ja tehdä tilauksia, ja tuottajat voivat lisätä omia tuotteitaan myyntiin.

Projekti on toteutettu kouluprojektina oppimistarkoituksessa JAMKin TICorporate Demo Labissa, eikä siitä ole varsinaista tuotantokäyttöön julkaistua versiota.

### 🧩 Oma roolini

Projektissa toimin useassa roolissa:

- UI/UX-suunnittelu (Figma)

- Frontend-kehitys

- Tietokantasuunnittelu

- Testaus

- Tiimityö ja kommunikointi

### 🛠️ Teknologiat ja työkalut

- Figma

- Frontend (HTML, CSS, JavaScript / Angular)

- MySQL

- Cypress & Vitest

- Git & GitHub

### 🧪 Testaus

Projektissa tehtiin saavutettavuus-, käytettävyys- ja yhteensopivuustestauksia sekä Cypress-testausta (4 käyttäjäpolkua). Lisäksi toteutettiin yksi yksikkötesti Vitestillä.

Sovellusta esiteltiin myös JAMKin Hakijan päivässä, jossa saatiin positiivista palautetta käyttäjiltä.

### 🌱 Mitä opin

- Iteratiivinen kehittäminen ja palautteen hyödyntäminen

- Käyttöliittymäsuunnittelun ja frontendin yhteys

- Tietokantarakenteen merkitys kokonaisuudessa

- Tiimityöskentelyn ja kommunikoinnin tärkeys

### 🔗 Linkit

GitHub-repo: (https://ramomaya.github.io/TC-Portfolio/)
